...
{
	std::vector<float> *selectedWeights = static_cast<std::vector<float> *>(clientData);

	for (int i = 0; i < NUM_BLENDSHAPES; i++)
	{
		selectedWeights[0][i] = 0.0f;
	}	
}
...
void addToObjectBuffer(Assignment1_vectors::CGObject *cg_object)
{
...
void addToIndexBuffer(Assignment1_vectors::CGObject *cg_object)
{
	int IBOindex = cg_object->startIBO;
...
Assignment1_vectors::CGObject loadObjObject(vector<objl::Mesh> meshes, bool addToBuffers, bool subjectToGravity, vec3 initTransformVector, vec3 initScaleVector, vec3 color, float coef, CGObject* parent)
{
	Assignment1_vectors::CGObject object = Assignment1_vectors::CGObject();
...
void createObjects()
{
	...
	if (usingLowRes)
	{
		dirName = "../Assignment1_vectors/meshes/Low-res Blendshape Model"; 
		neutralFileName = "Mery_jaw_open.obj";
	}
	else
	{
		dirName = "../Assignment1_vectors/meshes/High-res Blendshape Model";
	...
	for (auto const& filename : fileList) {
		...
		if (filename != neutralFileName)
		{
			...
			if (numberOfVertices == new_meshBlendshape[0].Vertices.size())
			{
				...
				face1.initialiseDeltaBlendshape(neutralFace.vpositions);
	...
	std::vector<float> customPositions;
...
//}
	...
	glUniform3f(glutils.objectColorLoc, 255/ 255.0f, 195/255.0f, 0.0f);
